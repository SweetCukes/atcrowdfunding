...
<button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${APP_PATH}/manage/user/add.htm' "><i class="glyphicon glyphicon-plus"></i> 新增</button>
<button type="button" class="btn btn-primary" style="float:right;margin-right:10px;" onclick="window.location.href='${APP_PATH}/manage/user/multiadd.htm' "><i class="glyphicon glyphicon-plus"></i> 批量新增</button>
<br>
 <hr style="clear:both;">
        ...
        <script type="text/javascript">
            $(function () {
			    ...
			    $("#queryBtn").click(function(){
			    	...
			    	if(queryText.val() == ""){
			    		layer.msg("请输入查询条件",{time:2000, icon:5, shitf:6},function(){
			    			queryText.focus();
			    		});
			    	}else{
			    		cond = true;
            ...
            function changePageno(pageno){
            	//window.location.href="${APP_PATH}/manage/user/list.htm?pageno="+pageno;
            	pageQuery(pageno);
            }
            var loadingIndex = -1;
            var cond = false;
            function pageQuery(pageIndex){
            	...
            	$.ajax({
            		type : "POST",
            		url  : "${APP_PATH}/manage/user/pageQuery.do",
            		data : jsonData,
            		beforeSend : function() {
            		...
            		success : function(result){
            			...
            			if(result.success){
            				...
            				$.each(userList,function(i,user){
            					...
            					userContent = userContent + '     <button type="button" onclick="window.location.href=\'${APP_PATH}/manage/user/assign.htm?id='+user.id+'\' "class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
            					userContent = userContent + '     <button type="button" onclick="window.location.href=\'${APP_PATH}/manage/user/edit.htm?pageno='+(pageIndex+1)+'&id='+user.id+'\' " class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
            				...
            				*/
            				...
            				layer.msg("用户分页数据查询失败",{time:2000, icon:5, shitf:6});
            			}
            		}
            	});
            }
            ...
            function deleteUser(id,loginacct){
            	layer.confirm("删除用户信息:"+loginacct+",是否继续？",  {icon: 3, title:'提示'}, function(cindex){
    			    ...
    			    $.ajax({
    			    	type : "POST",
    			    	url : "${APP_PATH}/manage/user/delete.do",
    			    	data : {
    			    		id : id
    			    	},
    			    	success : function(result){
    			    		if (result.success){
    			    			layer.msg("用户数据删除成功",{time:2000, icon:6},function(){
    			    				pageQuery(0);
    			    			});
    			    		}else{
    			    			layer.msg("用户数据删除失败",{time:2000, icon:5, shitf:6});
    			    		}
    			    	}
    			    });
            		layer.close(cindex);
            ...
            function deleteUsers(){
            	...
            	if( userBoxes.length == 0){
            		layer.msg("请选择删除的用户",{time:2000, icon:5, shitf:6});
            	}else{
            		...
            		layer.confirm("删除选择的信息,是否继续？",  {icon: 3, title:'提示'}, function(cindex){
                		...
                		$.ajax({
                			type  : "POST",
                			url     : "${APP_PATH}/manage/user/deletes.do",
                			data  : jsonData,
                			success : function (result){
                				if(result.success){
                					layer.msg("用户信息删除成功",{time:2000, icon:6},function(){
                						pageQuery(0);
                					});
                				}else{
                					layer.msg("用户信息删除失败",{time:2000, icon:5, shitf:6});
                				}
                			}
                		});
                		layer.close(cindex);
