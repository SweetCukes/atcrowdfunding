        ...
        <script type="text/javascript">
            ...
            $("#left2RightBtn").click(function(){
            	...
            	if(selItems.length == 0){
            		layer.msg("请选择需要分配的角色数据",{time:2000, icon:5, shitf:6});
            		return;
            	} else{
            		...
            		$.ajax({
            			type : "POST",
            			url    : "${APP_PATH}/manage/user/assignRole.do",
            			data : jsonData,
            			success : function(result){
            				if (result.success){
            					...
            					layer.msg("角色数据分配失败",{time:2000, icon:5, shitf:6});
            				}
            			}
            		});
            	}
 			...
 			$("#right2LeftBtn").click(function(){
            	...
            	// 判断元素的数量
            	if(selItems.length == 0){
            		layer.msg("请选择取消分配的角色数据",{time:2000, icon:5, shitf:6});
            		return;
            	} else{
              		var jsonData = {userid:"${user.id}"};
            		$.each(selItems,function(i,n){
            		...
            		$.ajax({
            			type : "POST",
            			url    : "${APP_PATH}/manage/user/unassignRole.do",
            			data : jsonData,
            			success : function(result){
            				if (result.success){
            					// 如果数量大于1，元素向右移动
