...
<button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${APP_PATH}/manage/role/add.htm'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
<br>
 <hr style="clear:both;">
	...
	<script src="${APP_PATH}/script/layer/layer.js"></script>
        <script type="text/javascript">
            ...
            function pageQuery(pageno) {
            	...
            	$.ajax({
            		url : "${APP_PATH}/manage/role/pageQuery.do",
            		type : "POST",
            		data : obj,
            		beforeSend : function() {
            			...
            			if ( result.success ) {
            				...
            				$.each(roleList, function(i, role){
             				   content = content +  '<tr>';
          	                   ...
          	                   content = content +  '  <td>';
          					   content = content +  '      <button type="button" onclick="window.location.href=\'${APP_PATH}/manage/role/assign.htm?id='+role.id+'\'"class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
          					   content = content +  '      <button type="button" onclick="window.location.href=\'${APP_PATH}/manage/role/edit.htm?pageno='+pageObj.pageno+'&id='+role.id+'\'" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
            	...
            	if( roleBoxes.length == 0){
            		...
            		layer.confirm("删除选择的信息,是否继续？",  {icon: 3, title:'提示'}, function(cindex){
                		...
                		$.ajax({
                			...
                			success : function (result){
                				if(result.success){
                					layer.msg("角色信息删除成功",{time:2000, icon:6},function(){
                						pageQuery(1);
                					});
                				}else{
